...
export const authApi: IAuthAPIService = {
  async login(username, password) {
    return axios.post<AuthResponse>(`${BASE}/login`, { username, password })
      .then(r => r.data).catch((e: AxiosError<ErrBody>) => apiError(e, "Login failed"));
  },
  async register(payload: RegisterPayload) {
    return axios.post<AuthResponse>(`${BASE}/register`, {
      ...
      .then(r => r.data).catch((e: AxiosError<ErrBody>) => apiError(e, "Registration failed"));
  },
  async logout() {
    return axios.post<{ success: boolean; message?: string }>(`${BASE}/logout`, {}, { headers: authHeader() })
      .then(r => r.data).catch((e: AxiosError<ErrBody>) => apiError(e, "Logout failed"));
  },
};
